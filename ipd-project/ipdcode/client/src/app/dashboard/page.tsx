...
const DashboardPage = () => {
  ...
  const handleFile = (file: File) => {
    ...
    if (isImage) fileType = 'image';
      ...
      // Check extension if MIME type not recognized
    ...
    // Reset all analysis states for new upload
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        formData.append(filePreview.type === 'image' ? 'image' : 'video', filePreview.file);
  ...
  const getDeepfakeScore = () => {
    if (filePreview?.type === 'image' && imageAnalysis) {
      const aiScore = imageAnalysis.predictions.find(p => p.label === 'AI-generated')?.score || 0;
      const avgFaceDistortion = imageAnalysis.face_distortion && imageAnalysis.face_distortion.length > 0 
        ? imageAnalysis.face_distortion.reduce((sum: number, face: any) => sum + (face.distortion_score || 0), 0) / imageAnalysis.face_distortion.length 
        : 0;
      
      // Dynamic weighting: if face distortion is high, give it more weight
      const faceWeight = avgFaceDistortion > 30 ? 0.4 : 0.3;
      const aiWeight = 1 - faceWeight;
      
      return Math.round(aiScore * 100 * aiWeight + avgFaceDistortion * faceWeight);
    }
    if (!metrics) return 0;
    ...
    return Math.round(
      videoMetrics.faceDistortion * 0.4 + 
      videoMetrics.lipSyncDeviation * 0.3 + 
      videoMetrics.frameConsistency * 0.2 + 
      videoMetrics.audioVideoMismatch * 0.1
    );
  };
  ...
  const getMetricsData = () => filePreview?.type === 'image' ? (
    imageAnalysis ? [
      { 
        ...
        value: (imageAnalysis.predictions.find(p => p.label === 'AI-generated')?.score || 0) * 100,
        color: getMetricColor((imageAnalysis.predictions.find(p => p.label === 'AI-generated')?.score || 0) * 100)
      },
      { 
        label: "Real Confidence", 
        value: (imageAnalysis.predictions.find(p => p.label === 'real')?.score || 0) * 100,
        color: getMetricColor((imageAnalysis.predictions.find(p => p.label === 'real')?.score || 0) * 100)
      },
      { 
        label: "Face Distortion", 
        value: imageAnalysis.face_distortion && imageAnalysis.face_distortion.length > 0 
          ? imageAnalysis.face_distortion.reduce((sum: number, face: any) => sum + (face.distortion_score || 0), 0) / imageAnalysis.face_distortion.length
          : 0,
        color: getMetricColor(imageAnalysis.face_distortion && imageAnalysis.face_distortion.length > 0 
          ? imageAnalysis.face_distortion.reduce((sum: number, face: any) => sum + (face.distortion_score || 0), 0) / imageAnalysis.face_distortion.length
          : 0)
      },
      { 
        label: "Overall Score", 
        value: imageAnalysis.best_score * 100,
        color: getMetricColor(imageAnalysis.best_score * 100)
      }
    ] : [
    { 
