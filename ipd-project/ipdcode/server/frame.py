...
def detect_frame_anomalies(video_path, skip_frames=5):
    ...
    if not cap.isOpened():
        print("ERROR: OpenCV could NOT open video!")
        raise Exception("OpenCV could not open video file.")
    print("DEBUG: OpenCV opened video successfully.")
    ...
    try:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                print(f"ERROR: Exception while processing frame {frame_count}: {e}")
        ...
        print(f"DEBUG: Finished processing {total_frames} frames.")
        return total_frames, abnormal_frames

    except Exception as e:
        print(f"ERROR: Exception in detect_frame_anomalies: {e}")
        cap.release()
        raise
